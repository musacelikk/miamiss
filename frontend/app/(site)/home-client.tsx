...
/**
 * Anasayfa gorunumu. Veri sunucuda cekilip prop olarak gelir — ilk HTML'de
 * hero metni ve video adresi hazir olur, tarayici indirmeye hemen baslar.
 */
export function HomeClient({
  ...
  if (fullBg) {
    const poster = hp.heroBackgroundImage ? imageUrl(hp.heroBackgroundImage) : null
    // Medya baska bir alan adindan (S3/CloudFront) geliyorsa baglantiyi onceden ac
    const mediaOrigin = (() => {
      const src = fullBg === "video" ? hp.heroBackgroundVideo : hp.heroBackgroundImage
      try {
          ...
          {fullBg === "video" ? (
            <video
              ...
              className={cn(
                ...
                poster && !videoReady ? "opacity-0" : "opacity-100",
              )}
            />
          ) : (
            !poster && (
